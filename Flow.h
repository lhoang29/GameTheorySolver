...
class Flow : public Visualization
{
      private:
      ...
      public:
                 ...
                 for (int i=0; i<numRow; i++)
                 {
                    for(int j=0; j<numCol; j++)
                    {
                       if(j+1 < numCol)
                       {
                          ...
                          //if going right
                          if(pm.getPayoff(numRow-1-i, j) > pm.getPayoff(numRow-1-i, j+1))
                          {
                             if(pm.getPayoff(numRow-1-i,j+1) == pm.getRowExtrema(true,numRow-1-i,true))
                             {
                          ...
                          if(pm.getPayoff(numRow-1-i, j) < pm.getPayoff(numRow-1-i, j+1))
                          {
                             if(pm.getPayoff(numRow-1-i,j) == pm.getRowExtrema(true,numRow-1-i,true))
                             {                  
                       ...
                       if(i+1 < numRow)
                       {
                          ...
                          //if going up
                          if(pm.getPayoff(numRow-1-i, j) < pm.getPayoff(numRow-2-i, j))
                          {
                             if(pm.getPayoff(numRow-2-i, j)==pm.getColExtrema(false,j,true))
                             {
                          ...
                          if(pm.getPayoff(numRow-1-i, j) > pm.getPayoff(numRow-2-i, j))
                          {
                             if(pm.getPayoff(numRow-1-i, j)==pm.getColExtrema(false,j,true))
                             {
                                 Point<int> p3((int)(leftBound + j*horizontalStep - 5), 
              ...
              virtual PointCollection<double> buildPoints(payoffMatrix& pm)
              {
                 ...
                 //Set indexes of the saddle points to later match and color them red
                 vector<int*> saddleResult = pm.getSaddlePoints();
